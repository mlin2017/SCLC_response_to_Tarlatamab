...
## becomes supplementary figure 5A
...
epi_sub <- subset(scrna.unintegrated,subset= final_anno %in% c("endothelial","fibroblast","epithelial"))
...
write.table(annotation, file="../../inferCNV/inputs/singleCell.annotation.txt",quote=FALSE,col.names = FALSE,row.names=TRUE,sep="\t")

 
# create the infercnv object
infercnv_obj = CreateInfercnvObject(raw_counts_matrix=counts,
                                    annotations_file="../../inferCNV/inputs/singleCell.annotation.txt",
                                    delim="\t",
...
infercnv_obj = infercnv::run(infercnv_obj,
                             ...
                             out_dir="../../inferCNV/outputNew",  # dir is auto-created for storing outputs
...
infercnv_obj <- readRDS("../../inferCNV/outputNew/run.final.infercnv_obj")
...
## epithelial_17 seem to be normal
...
############################
### use T, B cells as control
epi_sub <- subset(scrna.unintegrated,subset= final_anno %in% c("T","B","epithelial"))
...
write.table(annotation, file="../../inferCNV/inputs/singleCell.annotation.TBctrl.txt",quote=FALSE,col.names = FALSE,row.names=TRUE,sep="\t")

 
# create the infercnv object
infercnv_obj = CreateInfercnvObject(raw_counts_matrix=counts,
                                    annotations_file="../../inferCNV/inputs/singleCell.annotation.TBctrl.txt",
                                    delim="\t",
                                    gene_order_file="../../inferCNV/inputs/hg38_gencode_v27.txt",
                                    chr_exclude = c("chrX","chrY","chrM"),
...
infercnv_obj = infercnv::run(infercnv_obj,
                             cutoff=0.1,  # use 1 for smart-seq, 0.1 for 10x-genomics
                             out_dir="../../inferCNV/outputTB",  # dir is auto-created for storing outputs
                             cluster_by_groups=T,   # cluster
                             analysis_mode= "samples",
                             denoise=T,
                             HMM=F,
...
#### density plot for redefining positive cells
...
### final S7A only -A, -N, and -P due to small amount of -I and consistent with cohort B
...
## final 7C later modified in illustrator
...
## Final figure 7B further modified in illustrator
...
## final 2A which were modified further in illustrator
...
### finalize a few figures on 11-12-2025
...
VlnPlot2(epi_sub, features="invasion", group.by = "DLL3_group", lab_fill="DLL3",pt=F,show.mean=TRUE,
  width=0.5, stat.method = "wilcox.test",cols=c("limegreen","darkorange1"), label="p.format",label.size=5)+ 
  labs(title = NULL)+ ylab("UCell Signature Score") +
  ggtitle("Invasion Signature by DLL3 Group") +
  theme(axis.text.x = element_blank(),axis.text.y = element_text(size = 10) ,plot.title = element_text(size = 14,hjust=0.5),
        strip.text = element_blank() ) 
  

ggsave((paste0("../../plots/revision/",Sys.Date(),"_SCLC_invasion_score_by_DLL3_rebuttal.pdf")))

### invasion signature score in response only
...
# Warning: The following requested variables were not found: KRT6
epi_sub$CK_group[which(rowSums(expr_matrix > 0) > 0)] <- "pos"
epi_sub$CK_group <- factor(epi_sub$CK_group)

## negative

neg <- WhichCells(epi_sub, expression=DLL3_group=="neg" & EPCAM_group=="neg" & CK_group=="neg") #512
...
ggvenn(a2,fill_color=c("orange","blue","yellow"), show_percentage=FALSE,set_name_size=0,text_size=4) +
  ...
  annotate("text",x=0,y=0.6,label="(0.3%)") +
  annotate("text",x=0.8,y=0.4,label="(2%)") +
  annotate("text",x=-0.5,y=-0.2,label="(3.4%)") +
  annotate("text",x=0,y=0,label="(27.3%)") +
  annotate("text",x=0.5,y=-0.2,label="(52%)") +
  annotate("text",x=0,y=-0.8,label="(12.1%)") +
  annotate("text",x=1.6,y=1.6,label="EpCAM+") +
  annotate("text",x=1,y=-1.4,label="CK+") +
  annotate("text",x=-0.1,y=-1.8,label="Marker Neg 512(2.6%)") + ggtitle("Cohort B")+theme(plot.title = element_text(hjust = 0.5))

ggsave((paste0("../../plots/revision/",Sys.Date(),"_venn_EPCAM_DLL3_CK_neg_ggvenn_cohortB.pdf")))
