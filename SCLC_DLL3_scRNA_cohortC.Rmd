...
### final S9B (pseudobulk DLL3 across all subtypes)
...
### final S9B (pseudobulk ASCL1 across all subtypes)
...
### final S9A (ASCL1 vs DLL3 using all subtypes)
...
### final S9A (ASCL1 vs DLL3 using -A subtype only)
...
barplot(
  ...
  names.arg = paste0("PC", 1:20),   # 👈 adds PC1–PC20 labels
...
## define CK +/-
sclc$CK_group <- "neg"
expr_matrix <- FetchData(sclc, vars = c("KRT4","KRT5","KRT6","KRT8","KRT10","KRT13","KRT18"))
...
neg <- WhichCells(sclc, expression=DLL3_group=="neg" & EPCAM_group=="neg" & CK_group=="neg") #282
...
ggvenn(a2,fill_color=c("orange","blue","yellow"), show_percentage=FALSE,set_name_size=0,text_size=4) +
  ...
  annotate("text",x=0.8,y=0.4,label="(0.5%)") +
  annotate("text",x=-0.5,y=-0.2,label="(3.3%)") +
  annotate("text",x=0,y=0,label="(61.1%)") +
  annotate("text",x=0.5,y=-0.2,label="(29.9%)") +
  annotate("text",x=0,y=-0.8,label="(4.0%)") +
  annotate("text",x=1.6,y=1.6,label="EpCAM+") +
  annotate("text",x=1,y=-1.4,label="CK+") +
  annotate("text",x=-0.1,y=-1.8,label="Marker Neg 282(0.6%)") + ggtitle("Cohort C")+theme(plot.title = element_text(hjust = 0.5))

ggsave((paste0("../../manuscript/revision/",Sys.Date(),"_venn_EPCAM_DLL3_CK_neg_ggvenn_cohortC.pdf")))

### final supplementary S21 B
